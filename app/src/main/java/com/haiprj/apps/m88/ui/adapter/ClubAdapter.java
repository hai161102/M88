...
import com.haiprj.apps.m88.databinding.ItemTeamBinding;
import com.haiprj.apps.m88.model.ClubModel;
import com.haiprj.apps.m88.ui.activity.TeamInfoActivity;
...
public class ClubAdapter extends RecyclerView.Adapter<ClubAdapter.ClubHolder> {
    ...
    private final List<ClubModel> list = new ArrayList<>();
    ...
    public void update(List<ClubModel> list) {
        this.list.clear();
    ...
    public ClubHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ClubHolder(LayoutInflater.from(this.context).inflate(R.layout.item_team, parent, false));
    }
    ...
    public class ClubHolder extends RecyclerView.ViewHolder{
        ...
        public void load(ClubModel clubModel) {
            binding.teamImage.setImageResource(clubModel.getLogoRes());
            binding.teamName.setText(clubModel.getClubName());
            binding.infoBtn.setOnClickListener(v -> {
                TeamInfoActivity.start(context, clubModel);
            });
        }
    }
}
